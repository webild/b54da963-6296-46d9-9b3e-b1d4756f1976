...
const PushableButton = ({
    ...
    return (
        <button
            ...
            className={`group relative bg-black rounded border-none p-0 cursor-pointer outline-offset-4 
                disabled:cursor-not-allowed disabled:opacity-50 transition-all duration-200
                ${buttonTransform} ${className}`}
        >
            <span 
                className={`px-10 h-10 rounded flex items-center justify-center text-sm bg-blue text-white 
                    ${spanTransform} ${frontClassName}`}
            >
                {children || text}
            </span>
        </button>
